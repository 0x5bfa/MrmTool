...
2. Copy
   ...
   - `WinUIEditorCsWinRT.xml` and `WinUIEditor.xml` from `WinUIEditorCsWinRT\nuget` folder to `MrmTool\Resources\WinUIEdit` folder in this repository
3. For each arch copy `WinUIEditor.dll` from `_buildUwp\[arch]\Release\WinUIEditor\bin\WinUIEditor` to `MrmTool\Resources\WinUIEdit\[arch]` folder in this repository.
